...
const WeatherCard = ({ data }) => {
  ...
  return (
    <div className="text-center">
      ...
      <p className="text-md text-white/70 mt-1">{location.localtime}</p>

      <div className="flex flex-col items-center mt-4 mb-6">
        <img src={condition.icon} alt={condition.text} className="w-20 h-20" />
        <p className="text-white/80 text-sm mt-2">{condition.text}</p>
        <h1 className="text-[38px] font-extrabold mt-1">{temp_c}°C</h1>
      </div>

      <div className="grid grid-cols-3 gap-3 text-xs font-medium mt-4">
